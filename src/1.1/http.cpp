		...
		while (pAdapterInfo) {
			if (pAdapterInfo->IpAddressList.IpAddress.String[0] != '\0') {
				...
				if (ipAddress != "127.0.0.1" && ipAddress != "0.0.0.0") {
					WSACleanup();
					return ipAddress;  // Return the first valid non-loopback IP address
				}
			}
...
std::string getFileExtension(const std::string& filename) {
	...
	return filename.substr(dotPos + 1); // Extract extension
}
...
void session(boost::asio::ip::tcp::socket socket) {
	try {
		...
		boost::beast::http::write(socket, res);
		socket.shutdown(boost::asio::ip::tcp::socket::shutdown_send);
		std::cout << "Error 404 encountered" << std::endl;
	}
		...
		boost::beast::http::write(socket, res);
		socket.shutdown(boost::asio::ip::tcp::socket::shutdown_send);
		std::cout << "Error 400 encountered" << std::endl;
	}
...
int main(int argc, char* argv[]) {
	...
	try {
		...
		std::cout << "URL: http://localhost:"+std::to_string(port)+" or "+getIpAddress()+":"+std::to_string(port) << std::endl;
		while (true) {
